...
# Defaults (override via: make build OS=ubuntu VERSION=2.0.12)
OS ?= ubuntu
VERSION ?= 2.0.12
...
help: ## 列出可用命令与说明
	@echo "用法: make <target> [OS=ubuntu|centos|alpine] [VERSION=2.0.0~2.0.12] [PLATFORMS=linux/amd64,linux/arm64] [IMAGE_REPO=...] [IMAGE_TAG_PREFIX=dood]"
	@echo
	@echo "可用目标:"
	...
	@printf "  VERSION           default: 2.0.12                         current: %s\n" "$(VERSION)"
	...
	@echo "示例:"
	@echo "  make builder"
	@echo "  make load OS=alpine VERSION=2.0.12"
	@echo "  make push OS=centos VERSION=2.0.0"
	@echo "  make build OS=ubuntu VERSION=2.0.12 PLATFORMS=linux/arm64 ONEPANEL_TYPE=pro"
...
matrix-push: builder ## 循环构建并推送 OS×版本 矩阵
	@versions="2.0.0 2.0.1 2.0.2 2.0.3 2.0.4 2.0.5 2.0.6 2.0.7 2.0.8 2.0.9 2.0.10 2.0.11 2.0.12"; \
